...
async function loadStats(): Promise<Stats> {
  ...
  const [
    ...
    db.collection("soldTickets").aggregate([
      { $match: { status: "paid" } },
      { $group: { _id: null, total: { $sum: "$amount" } } },
    ]).toArray(),
    db.collection("soldTickets").countDocuments({ status: "paid" }),
    db.collection("soldTickets").countDocuments({ status: "paid", paidAt: { $gte: startOfDay } }),
    db.collection("soldTickets").countDocuments({ status: "paid", scannedAt: { $exists: true } }),
    db.collection("soldTickets").countDocuments({ status: "paid", scannedAt: { $gte: startOfDay } }),
    db.collection("soldTickets").countDocuments({ status: "pending" }),
    db.collection("tickets").countDocuments({}),
    db.collection("messageJobs").countDocuments({ status: { $in: ["pending", "processing"] } }),
    db.collection("messageJobs").countDocuments({ status: "failed" }),
    db.collection("tickets").aggregate([
      { $group: { _id: null, remaining: { $sum: "$quantity" } } },
    ]).toArray(),
  ]);

  return {
    revenueGel: Number(revenueAgg[0]?.total ?? 0),
    ticketsSold,
    ticketsToday,
    scannedCount,
    scannedToday,
    ...
    emailsPending,
    emailsFailed,
    totalRemaining: Number(inventoryAgg[0]?.remaining ?? 0),
